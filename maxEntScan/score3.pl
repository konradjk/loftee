...
if ($read_from_file) {
  ...
  while(<FILE>) {
    ...
    if (/^\s*$/) { #discard blank lines;
      ...
      print $str."\t";
      $str = uc($str);
      print sprintf("%.2f", &log2(&score_consensus_acceptor($str)*&max_ent_score(&get_rest_acceptor($str),\@metables)))."\n";
    }
  }
}
